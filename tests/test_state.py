...
def result(n: str, step: str, status: str, out_root: Path, attempt: int = 1) -> StepResult:
    return StepResult(
        n=n,
        step=step,
        status=status,
        ...
        attempt=attempt,
        seconds=1.5,
        workdir=out_root / "runs" / n / step / "attempt{}".format(attempt),
        started="2026-08-06T10:00:00",
    )
...
class JournalTest(unittest.TestCase):
    ...
    def test_запись_и_чтение(self):
        with Journal(self.path, self.tmp) as journal:
            journal.append(result("103m5", "create", STATUS_OK, self.tmp))
            journal.append(result("104m5", "create", STATUS_FAIL, self.tmp))
        records = read_records(self.path)
        self.assertEqual(len(records), 2)
        self.assertEqual(records[0]["n"], "103m5")
    ...
    def test_путь_в_журнале_относительный(self):
        ...
        self.assertEqual(record["dir"], str(Path("runs") / "103m5" / "create" / "attempt1"))

    def test_ключ_различает_шаги(self):
        with Journal(self.path, self.tmp) as journal:
            journal.append(result("103m5", "create", STATUS_OK, self.tmp))
            journal.append(result("103m5", "enable", STATUS_FAIL, self.tmp))
        records = read_records(self.path)
        self.assertEqual(completed_keys(records), {("103m5", "create")})
        self.assertEqual(failed_keys(records), {("103m5", "enable")})

    def test_ключ_различает_кластеры(self):
        # 103 на m4 и на m5 — разная работа, и путать их нельзя.
        with Journal(self.path, self.tmp) as journal:
            journal.append(result("103m4", "create", STATUS_OK, self.tmp))
            journal.append(result("103m5", "create", STATUS_FAIL, self.tmp))
        records = read_records(self.path)
        self.assertEqual(completed_keys(records), {("103m4", "create")})
        self.assertEqual(failed_keys(records), {("103m5", "create")})

    def test_успех_после_провала_считается_выполненным(self):
        with Journal(self.path, self.tmp) as journal:
            journal.append(result("103m5", "create", STATUS_FAIL, self.tmp, attempt=1))
            journal.append(result("103m5", "create", STATUS_OK, self.tmp, attempt=2))
        records = read_records(self.path)
        self.assertEqual(completed_keys(records), {("103m5", "create")})
    ...
    def test_таймаут_считается_провалом(self):
        with Journal(self.path, self.tmp) as journal:
            journal.append(result("109m4", "create", STATUS_TIMEOUT, self.tmp))
        records = read_records(self.path)
        self.assertEqual(failed_keys(records), {("109m4", "create")})
